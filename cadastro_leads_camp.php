...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $id_act_lead = (isset($_POST["id"]) && $_POST["id"] != null) ? $_POST["id"] : "";
    ...
    $usuario = (isset($_POST["usuario"]) && $_POST["usuario"] != null) ? $_POST["consultor"] : "";
    ...
    $id_act_lead_face = (isset($_POST["id_face"]) && $_POST["id_face"] != null) ? $_POST["id_face"] : "";
...
} else if (!isset($id_act_lead)) {
    // Se não se não foi setado nenhum valor para variável $id_act_lead
    $id_act_lead = (isset($_GET["id"]) && $_GET["id"] != null) ? $_GET["id"] : "";
    $nome = NULL;
    $email = NULL;
    $celular = NULL;
    $token_campanha = NULL;
...
if (isset($_REQUEST["act_lead"]) && $_REQUEST["act_lead"] == "save" && $nome != "") {
    try {
        if ($id_act_lead != "") {
        ...
        $stmt->bindParam(10, $id_act_lead_face);
...
if (isset($_REQUEST["act_lead"]) && $_REQUEST["act_lead"] == "upd" && $id_act_lead != "") {
    try {
        $stmt = $conexao->prepare("SELECT * FROM leads WHERE id = ?");
        $stmt->bindParam(1, $id_act_lead, PDO::PARAM_INT);
        if ($stmt->execute()) {
...
if (isset($_REQUEST["act_lead"]) && $_REQUEST["act_lead"] == "del" && $id_act_lead != "") {
    try {
...
<!DOCTYPE html>
    <html>
        ...
        <body>
            <form action="?act_lead=save" method="POST" name="form1" >
                <h1>Cadastro de leads</h1>
                <hr>
                <input type="hidden" name="id" <?php

                // Preenche o id no campo id com um valor "value"
                if (isset($id_act_lead) && $id_act_lead != null || $id_act_lead != "") {
                    echo "value=\"{$id_act_lead}\"";
                }
                ?> />
                ...
                <input type="hidden" name="consultor" value="<?php
                echo $user_id; ?>"  />
                ...
                <input type="hidden" name="titulo_campanha" <?php

                // Preenche o id no campo token_campanha com um valor "value"
                if (isset($titulo_campanha) && $titulo_campanha != null || $titulo_campanha != "") {
                    echo "value=\"{$titulo_campanha}\"";
                }
                ?>  />
               ...
               if (isset($id_act_lead_face) && $id_act_lead_face != null || $id_act_lead_face != "") {
                   echo "value=\"{$id_act_lead_face}\"";
               }
               ?> />
            ...
            <table class="table table-striped table table-striped mdl-data-table mdl-js-data-table mdl-data-table--selectable" border="1" width="100%">
                ...
                try {
                    $stmt = $conexao->prepare("SELECT * FROM leads where consultor ='$user_id' ");
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>";
                            echo "<td>".$rs->nome."</td><td>".$rs->email."</td><td>".$rs->celular
                                       ."</td><td><center><a href=\"https://leadsback.com.br/app/74be16979710d4c4e7c6647856088456?act_lead=upd&id=".$rs->id."\">[Editar]</a>"
                                       ."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                                       ."<a href=\"https://leadsback.com.br/app/74be16979710d4c4e7c6647856088456?act_lead=del&id=".$rs->id."\">[Excluir]</a></center></td>";
                            echo "</tr>";
                        }
                    } else {
