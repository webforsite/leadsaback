...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $nome != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE leads SET nome=?, email=?, celular=?, token_campanha=?, consultor=?, usuario=?, data=?, hora=?, whats=?, id_face=?, link_face=?, interesse=?, status=?, feedback=?, cep=?, data_nascimento=?, cidade=?, estado=?, logradouro=?, titulo_campanha=?, prodServ=?, quantidade=?, id_associado=?, desconto=?, codigo_produto=? WHERE id = ?");
            $stmt->bindParam(26, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO leads (nome, email, celular, token_campanha, consultor, usuario, data, hora, whats, id_face, link_face, interesse, status, feedback, cep, data_nascimento, cidade, estado, logradouro, titulo_campanha, prodServ, quantidade, id_associado, desconto, codigo_produto) VALUES (?, ?, ?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ? ,? ,? ,? ,? ,? ,? ,? , ?, ?, ?)");
        }
        ...
        $stmt->bindParam(23, $id_associado);
    ...
    <html>
        ...
        <body>
            <form action="?act=save" method="POST" name="form1" >
                ...
                <input type="hidden" name="id_associado" value="<?php echo $user_id; ?>" />
            ...
            <table class="table table-striped table table-striped mdl-data-table mdl-js-data-table mdl-data-table--selectable" border="1" width="100%">
                ...
                try {
                    $stmt = $conexao->prepare("SELECT * FROM leads where usuario='$user_login' ORDER by id desc limit 30 ");
                    if ($stmt->execute()) {
