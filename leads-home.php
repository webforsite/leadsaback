<?php
/**
* CRUD PDO - Meus Leads - 11/09/2018
* Rogério Mário
*/
// GLOBAL INFORMAÇÕES
global $current_user;
...
try {
if ($id != "") {
$stmt = $conexao->prepare("UPDATE cadastro_leads SET nome=?, whats=?, feedback=? WHERE id = ?");
$stmt->bindParam(4, $id);
} else {
$stmt = $conexao->prepare("INSERT INTO cadastro_leads (nome, whats, feedback) VALUES (?, ?, ?)");
}
...
echo '
<div class="progress">
  <div role="progressbar" class="progress-bar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: 60%;">OFFLINE 60%</div>
</div>';
echo  '
  ...
  <tr class="warning">
    ...
    <div class="media-left media-middle media-middle">
                                    <a href="#" data-toggle="tooltip" data-placement="top" title="" data-original-title="">
                                        <div class="avatar"> <img  onerror="imageError(this)" class="media-object img-circle" src="https://graph.facebook.com/'. $link_face . '/picture" alt="Avatar"><i class="avatar-status avatar-status-bottom  bg-success b-brand-gray-darker"></i> </div>
                                    </a>
    ...
    <td>'. $whats .'</td>
    <td>
      <a href="'. $link_face .'" target="_blank">
      ...
      <a href="https://api.whatsapp.com/send?phone=55'.$whats.'&text=Ol%C3%A1,!" target="_blank">
        <svg
    ...
    <td>
        ...
        <div class="">
          <select name="status" style="height: 35px!important" class="form-control">
              ...
              <ul class="nav nav-pills nav-stacked">
                ...
                <li role="presentation">
                  <a href="/app/74be16979710d4c4e7c6647856088456?act=upd&amp;id='. $id .'/'. $token .'" class="text-muted">
                    <i class="fa fa-fw fa-circle-o text-info m-r-1"></i> Editar
                  </a>
                </li>
                <li role="presentation">
...
// READ TABLE
try {
$stmt = $conexao->prepare("SELECT * FROM leads where data BETWEEN CURDATE() - INTERVAL 1 DAY AND CURDATE()  and (consultor ='$user_id') and (status='online') order by id desc ");
if ($stmt->execute()) {
...
echo "
    ...
    <div class='media-left media-middle media-middle'>
                                    <a href='#' data-toggle='tooltip' data-placement='top' title='' data-original-title=''>
                                        <div class='avatar'> <img  onerror='imageError(this)' class='media-object img-circle' src='https://graph.facebook.com/". $rs->link_face . "/picture' alt='Avatar'><i class='avatar-status avatar-status-bottom  bg-success b-brand-gray-darker'></i> </div>
                                    </a>
                                </div><div style='margin-left: 50px; margin-top: -35px'>
    ...
    <td>
      ...
      <a href='https://api.whatsapp.com/send?phone=55".$rs->whats."&text=Ol%C3%A1,!' target='_blank'>
        <svg
          xmlns='http://www.w3.org/2000/svg' width='25px' fill='#2dbda8' viewBox='0 0 448 512'>
          <path d='M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z'></path>
        </svg>
    ...
    <td>
      <div style='margin-top: 7px'>
        <svg class='status-svg' id='svg-online'
